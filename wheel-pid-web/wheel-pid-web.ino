...
enum HeadingPhase : uint8_t {
  HEADING_IDLE = 0,
  HEADING_MOVE = 1,
  HEADING_HOLD_ACTIVE = 2,
  HEADING_SETTLED = 3,
};
...
CascadePidState headingPid = {0.90f, 0.00f, 0.05f, 0.0f, 0.0f, 0.0f};
...
float headingCorrMaxRpm = 10.0f;
int8_t headingCorrSign = -1;
float headingDeadbandDeg = 2.0f;
float headingEnableBaseRpm = 3.0f;
bool autoLockHeadingOnStep = true;
bool holdHeadingAtStop = true;
float headingHoldMaxRpm = 6.0f;
float headingHoldDeadbandDeg = 1.2f;
CascadePidState headingHoldPid = {0.45f, 0.00f, 0.10f, 0.0f, 0.0f, 0.0f};
float headingHoldMinRpm = 0.0f;
float headingHoldEnterDeg = 4.0f;
float headingHoldExitDeg = 1.2f;
float headingHoldExitRateDps = 1.5f;
float headingHoldKickErrDeg = 8.0f;
float headingHoldKickGyroDps = 4.0f;
float headingCorrSlewRpmPerSec = 25.0f;
bool headingHoldActive = false;
float headingHoldStableMs = 0.0f;
float headingCorrCmdPrev = 0.0f;
HeadingPhase headingPhase = HEADING_IDLE;
float headingCorrTargetRpm = 0.0f;
float headingShapedErrorDeg = 0.0f;
float headingPTermRpm = 0.0f;
float headingDTermRpm = 0.0f;
...
float signedDeadband(float value, float deadband) {
  float mag = fabsf(value);
  if (mag <= deadband) return 0.0f;
  return signNonZero(value) * (mag - deadband);
}

float slewToward(float current, float target, float ratePerSec, float dt) {
  float step = max(0.0f, ratePerSec) * dt;
...
void resetHighLevelControllers() {
  ...
  headingPhase = HEADING_IDLE;
  headingCorrTargetRpm = 0.0f;
  headingShapedErrorDeg = 0.0f;
  headingPTermRpm = 0.0f;
  headingDTermRpm = 0.0f;
...
void updateHighLevelTargets(float dt) {
  ...
  if (imuOk) {
    ...
    if (movingPhase) {
      ...
      headingPhase = HEADING_MOVE;
      headingShapedErrorDeg = errMove;
      headingPTermRpm = headingPid.kp * errMove;
      headingDTermRpm = -headingPid.kd * imuGyroDps;

      // Damped move correction: P on heading error, D on measured yaw-rate.
      float uMove = headingPTermRpm + headingDTermRpm;
      ...
      float exitDeg = constrain(headingHoldExitDeg, 0.3f, 10.0f);
      float enterDeg = max(headingHoldEnterDeg, exitDeg + 1.0f);
      enterDeg = constrain(enterDeg, exitDeg + 0.2f, 30.0f);
      float absErr = fabsf(rawHeadingError);
      float absRate = fabsf(imuGyroDps);

      headingErrorDeg = rawHeadingError;

      if (!headingHoldActive && absErr >= enterDeg) {
      ...
      if (headingHoldActive) {
        headingPhase = HEADING_HOLD_ACTIVE;

        bool settleCandidate = absErr <= exitDeg && absRate <= headingHoldExitRateDps;
        if (settleCandidate) {
          headingHoldStableMs += dt * 1000.0f;
          corrTarget = 0.0f;

          if (headingHoldStableMs >= 250.0f) {
            headingHoldActive = false;
            headingPhase = HEADING_SETTLED;
            resetCascadePid(headingHoldPid);
          }
        } else {
          headingHoldStableMs = 0.0f;

          float errHold = signedDeadband(rawHeadingError, exitDeg);
          headingShapedErrorDeg = errHold;
          headingPTermRpm = headingHoldPid.kp * errHold;
          headingDTermRpm = -headingHoldPid.kd * imuGyroDps;
          headingHoldPid.integral = 0.0f;

          float uHold = (headingPTermRpm + headingDTermRpm) * (float)headingCorrSign;
          corrTarget = constrain(uHold, -headingHoldMaxRpm, headingHoldMaxRpm);

          // Hold Min is a stiction assist only for large, slow errors, not near the settle band.
          if (headingHoldMinRpm > 0.0f &&
              absErr >= enterDeg &&
              absRate <= headingHoldKickGyroDps &&
              fabsf(corrTarget) > 0.001f &&
              fabsf(corrTarget) < headingHoldMinRpm) {
            corrTarget = signNonZero(corrTarget) * headingHoldMinRpm;
          }
    ...
    headingCorrTargetRpm = corrTarget;
    corr = slewToward(headingCorrCmdPrev, corrTarget, headingCorrSlewRpmPerSec, dt);
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="row" style="margin-top:8px">
      ...
      <label>Head Kp</label><input id="head_kp" type="number" value="0.90" step="0.05" onchange="applyMotionConfig()" />
      <label>Head Ki</label><input id="head_ki" type="number" value="0.00" step="0.01" onchange="applyMotionConfig()" />
      <label>Head Kd</label><input id="head_kd" type="number" value="0.05" step="0.01" onchange="applyMotionConfig()" />
      <label>Max Head Corr RPM</label><input id="head_max" type="number" value="10" step="1" min="0" max="60" onchange="applyMotionConfig()" />
      ...
      <label>Hold Max Corr RPM</label><input id="head_hold_max" type="number" value="6" step="0.5" min="0" max="60" onchange="applyMotionConfig()" />
      <label>Hold Min Corr RPM</label><input id="head_hold_min" type="number" value="0.0" step="0.1" min="0" max="60" onchange="applyMotionConfig()" />
      <label>Move Deadband (deg)</label><input id="head_db" type="number" value="2.0" step="0.1" min="0" max="30" onchange="applyMotionConfig()" />
      <label>Hold Deadband (deg)</label><input id="head_hold_db" type="number" value="1.2" step="0.1" min="0" max="30" onchange="applyMotionConfig()" />
      <label>Hold Kp</label><input id="head_hold_kp" type="number" value="0.45" step="0.05" onchange="applyMotionConfig()" />
      <label>Hold Ki (off)</label><input id="head_hold_ki" type="number" value="0.00" step="0.01" onchange="applyMotionConfig()" />
      <label>Hold Kd</label><input id="head_hold_kd" type="number" value="0.10" step="0.01" onchange="applyMotionConfig()" />
      ...
      <label>Corr Slew (rpm/s)</label><input id="head_slew" type="number" value="25" step="1" min="0" max="400" onchange="applyMotionConfig()" />
...
async function refresh(){
  ...
  document.getElementById('motionStatus').textContent = `Position: target=${f(d.position.targetCm,2)} cm (${f(d.position.targetCounts,0)} cnt) | current=${f(d.position.currentCm,2)} cm (${f(d.position.currentCounts,0)} cnt) | error=${f(d.position.errorCm,2)} cm (${f(d.position.errorCounts,0)} cnt) | tol=${f(d.position.tolCm,2)} cm | cmd=${f(d.position.cmdRpm,2)} rpm | done=${d.position.done ? 'YES' : 'NO'} | MoveHeadPID=(${f(d.heading.kp,2)}, ${f(d.heading.ki,2)}, ${f(d.heading.kd,2)}) | HoldHeadPID=(${f(d.heading.holdKp,2)}, ${f(d.heading.holdKi,2)}, ${f(d.heading.holdKd,2)}) | Hold=${d.heading.holdAtStop ? 'ON' : 'OFF'} phase=${d.heading.phase} active=${d.heading.holdActive ? '1' : '0'} stable=${f(d.heading.holdStableMs,0)}ms | hold min/max=${f(d.heading.holdMinRpm,1)}/${f(d.heading.holdMaxRpm,1)} db=${f(d.heading.holdDeadbandDeg,1)}deg`;
  document.getElementById('imuStatus').textContent = `IMU: ${d.imu.ok ? 'OK' : 'NOT FOUND'} | yaw=${f(d.imu.yawDeg,2)} deg | gyroZ=${f(d.imu.gyroDps,2)} dps | accX=${f(d.imu.accXg,3)}g | accY=${f(d.imu.accYg,3)}g | accXY=${f(d.imu.accXYg,3)}g | bias=${f(d.imu.biasDps,3)} dps | headingErr=${f(d.heading.errorDeg,2)} deg | shaped=${f(d.heading.shapedErrorDeg,2)} deg | corr=${f(d.heading.corrRpm,2)} rpm target=${f(d.heading.corrTargetRpm,2)} rpm | P=${f(d.heading.pTermRpm,2)} D=${f(d.heading.dTermRpm,2)}`;
...
String jsonData() {
  ...
  s += "\"phase\":" + String((int)headingPhase) + ",";
  s += "\"corrTargetRpm\":" + String(headingCorrTargetRpm, 3) + ",";
  s += "\"shapedErrorDeg\":" + String(headingShapedErrorDeg, 3) + ",";
  s += "\"pTermRpm\":" + String(headingPTermRpm, 3) + ",";
  s += "\"dTermRpm\":" + String(headingDTermRpm, 3) + ",";
...
void handleMotionConfig() {
  ...
  if (server.hasArg("hhki")) headingHoldPid.ki = 0.0f;
  ...
  if (server.hasArg("hholddb")) {
    ...
    headingHoldExitDeg = constrain(headingHoldDeadbandDeg, 0.3f, 10.0f);
  }
  ...
  if (holdOnlyRunning) {
    resetCascadePid(headingPid);
    resetCascadePid(headingHoldPid);
    headingCorrCmdPrev = headingCorrRpm;
    headingHoldActive = false;
    headingHoldStableMs = 0.0f;
    headingPhase = HEADING_SETTLED;
  } else {
